...
2. [The Canonical Input Contract](#2-the-canonical-input-contract)
3. [A Taxonomy of Rules](#3-a-taxonomy-of-rules)
4. [Library Structure and Package Design](#4-library-structure-and-package-design)
5. [The Exception Model in Rego](#5-the-exception-model-in-rego)
6. [Testing and CI Discipline](#6-testing-and-ci-discipline)
7. [The Rule Authoring Lifecycle](#7-the-rule-authoring-lifecycle)
8. [Honest Limits of Rego](#8-honest-limits-of-rego)
9. [Performance and Scale](#9-performance-and-scale)
10. [Observability of the Checker Itself](#10-observability-of-the-checker-itself)
11. [Worked Example: Conditional Access Tightening, End to End](#11-worked-example-conditional-access-tightening-end-to-end)
12. [Adoption Hurdles and Honest Tradeoffs](#12-adoption-hurdles-and-honest-tradeoffs)
...
## 2. The Canonical Input Contract

### 2.1 The CDM as Input Schema
...
### 2.2 Schema Versioning and Input Validation as a First-Class Category
...
Input validation rules belong in their own package, evaluated first, and treated as hard-fail:
...
### 2.3 Graceful Degradation vs. Fail-Closed
...
## 4. Library Structure and Package Design
...
### 4.3 The Role of the `data` Document
...
### 5.3 The Deny-by-Default-with-Validated-Exception Pattern
...
Domain rules import and call this predicate:
...
### 5.4 Exception Precedence and Malformed Exceptions

Exception records are themselves subject to validation rules in `guardrails.schema`. A malformed exception — missing `approved_by`, unparseable `expires_epoch`, or a `rule_id` that does not correspond to any known rule in the library — is flagged as a schema violation and does not activate. This is the correct behavior: a malformed exception silently activating would be indistinguishable from a deliberate bypass attempt in an audit review.
...
### 5.5 Surfacing the Exception Identifier in Audit Output
...
### 6.1 Why Testing Guardrail Rules Is Not Optional
...
### 6.2 Test Fixtures and the `data` Document in Tests
...
### 6.4 Property-Style Testing for Complex Inputs
...
### 7.2 The Advisory → Enforcing Promotion Path

A new rule entering the library starts in the advisory tier (Section 3.4): it emits findings but does not block pull requests. The observation period should be at minimum two weeks in a representative environment — long enough to see the rule fire against real proposed changes and to validate that the firing rate and the scope of affected records match the author's expectation. Rules with unexpectedly high firing rates are usually over-broad. Rules that never fire during the observation period may have an input field path error.
...
### 7.4 The Library as a Maker–Checker Subject
...
## 8. Honest Limits of Rego

Section 5.4 of the parent paper stated the limit plainly: multi-hop constraints are genuinely a graph-analysis problem that Rego alone does not express elegantly. This section develops that observation into a practical taxonomy of what Rego handles well, what it handles awkwardly, and what must be routed elsewhere.

### 8.1 What Rego Handles Well

Rego is well-suited to constraints that evaluate a single record against a set of conditions. The rules in Sections 2 through 5 of this paper are representative: check that a required field exists, check that a value falls within an allowed set, check that a combination of attributes satisfies a policy, check that an exception is present and valid. These constraints are expressible with simple unification and negation, they evaluate quickly, and they produce deterministic results.

Rego also handles cross-record correlation reasonably well when the correlation is shallow — one join between the input record and a data document, or at most two. The `IAM-SCOPE-001` rule in Section 4.3 joins the input record against `data.cmdb.hosts` and `data.cmdb.groups` in a single rule body, and this is the complexity ceiling at which Rego remains both readable and maintainable.

### 8.2 What Rego Handles Awkwardly
...
### 8.3 Routing to Specialized Engines
...
### 9.1 What Governs Evaluation Time
...
### 9.3 One Big Query vs. Many Small Queries
...
## 10. Observability of the Checker Itself

### 10.1 Structured Denial Messages
...
### 10.3 Metrics on Rule Firing Rates and Their Use
...
## 11. Worked Example: Conditional Access Tightening, End to End

### 11.1 The Scenario

This section traces the exact Example A workflow from the parent paper through the Rego rules that evaluate it — including a denial cycle that the parent paper elided. In the parent paper, CA-IDENT-042 passes the Checker with five rules evaluated and zero violations. The full workflow includes one earlier revision that did not pass. That revision is the part of the workflow that earns the architecture's safety claim.

To recap the scenario: the Identity sub-agent, running its scheduled 30-day sign-in posture review, identifies 62 accounts that are currently authenticating via legacy protocols (POP, IMAP, SMTP basic) and that have modern clients available. It drafts a Conditional Access policy — CA-IDENT-042 — to block legacy authentication for those accounts and opens a pull request into the CMDB.

### 11.2 The Initial Proposal
...
### 11.3 The Five Rules That Evaluate It
...
### 11.4 The Denial and Its Structured Output
...
## 12. Adoption Hurdles and Honest Tradeoffs
